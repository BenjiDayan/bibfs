...
def process_graph(graph_name):
    df = bbbfa.do_real_fake_comparison(graph_name, n_pairs=5000)
    p = '../M3_ext_val_data/real_fake_output/'
    os.makedirs(p, exist_ok=True)
    output_path = p + graph_name + '.csv'
    df.to_csv(output_path, index=False)
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=14) as executor:
        results = list(tqdm(executor.map(process_graph_blasius, graph_names), total=len(graph_names)))